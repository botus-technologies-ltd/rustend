...
#[derive(Debug, Clone)]
pub struct TCashConfig {
    pub client_id: String,
    pub client_secret: String,
    pub merchant_id: String,
    pub environment: TCashEnvironment,
}
#[derive(Debug, Clone, Copy)]
pub enum TCashEnvironment {
    Sandbox,
    Production,
}
...
impl TCashConfig {
    pub fn new(
        ...
        Self {
            ...
            environment: TCashEnvironment::Sandbox,
        }
    }
    pub fn production(mut self) -> Self {
        self.environment = TCashEnvironment::Production;
        self
    }
}

#[derive(Clone)]
pub struct TCashGateway {
    _config: TCashConfig,
    _client: reqwest::Client,
}
impl TCashGateway {
    pub fn new(config: TCashConfig) -> Self {
        Self {
            _config: config,
...
impl PaymentGateway for TCashGateway {
    ...
    async fn create_payment(
        ...
        let mut intent = PaymentIntent::new(PaymentProvider::TCash, amount);
        intent.customer = customer;
        intent.description = description;
        intent.metadata = metadata;
        intent.next_action = Some(PaymentAction {
            action_type: PaymentActionType::Otp,
            data: serde_json::json!({ "message": "Enter PIN" }),
        });
        Ok(intent)
    }
    ...
    async fn refund(&self, request: RefundRequest) -> PaymentResult<RefundResult> {
        Ok(RefundResult {
            success: true,
            refund_id: Some(format!("REF_{}", uuid::Uuid::new_v4())),
            status: PaymentStatus::Refunded,
            amount: request.amount.unwrap_or(0),
        })
    }
    async fn create_customer(&self, _customer: Customer) -> PaymentResult<String> {
        Ok(format!("TCASH_CUS_{}", uuid::Uuid::new_v4()))
    }
    async fn get_customer(&self, _customer_id: &str) -> PaymentResult<Customer> {
        Ok(Customer::new())
    }
    async fn attach_payment_method(
        ...
        Ok(format!("TCASH_PM_{}", uuid::Uuid::new_v4()))
    }
    ...
    async fn update_subscription(
        ...
        Ok(Subscription::new(
            request.plan_id.as_deref().unwrap_or("default"),
            "customer_123",
    ...
    async fn get_subscription(&self, _subscription_id: &str) -> PaymentResult<Subscription> {
        Ok(Subscription::new(
            "plan_123",
            "customer_123",
            &SubscriptionPlan::new(
    ...
    async fn get_payout(&self, _payout_id: &str) -> PaymentResult<Payout> {
        Ok(Payout::new(
            Amount::new(500000, "IDR"),
            "recipient_123",
            RecipientType::Individual,
            PaymentProvider::TCash,
    ...
    async fn create_batch_payout(&self, payouts: Vec<Payout>) -> PaymentResult<BatchPayout> {
        Ok(BatchPayout::new(PaymentProvider::TCash, payouts))
    }
    ...
    async fn get_balance(&self, account_id: &str) -> PaymentResult<WalletBalance> {
        Ok(WalletBalance {
            ...
            available: Amount::new(10000000, "IDR"),
            pending: Amount::new(1000000, "IDR"),
    ...
    fn parse_webhook_event(&self, payload: &[u8]) -> PaymentResult<WebhookEvent> {
        ...
        Ok(WebhookEvent {
            ...
            provider: PaymentProvider::TCash,
            data: value,
