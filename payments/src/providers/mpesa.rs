...
impl MpesaConfig {
    ...
    pub fn production(mut self) -> Self {
        self.environment = MpesaEnvironment::Production;
        self
    }
}
...
impl PaymentGateway for MpesaGateway {
    ...
    async fn create_payment(
        ...
        let mut intent = PaymentIntent::new(PaymentProvider::Mpesa, amount);
        intent.customer = customer;
        intent.description = description;
        intent.metadata = metadata;
        intent.next_action = Some(PaymentAction {
            action_type: PaymentActionType::Otp,
            data: serde_json::json!({ "message": "Enter PIN on your phone" }),
        });
        Ok(intent)
    }
    ...
    async fn cancel_payment(&self, _payment_intent_id: &str) -> PaymentResult<TransactionResult> {
        Ok(TransactionResult::failed("Payment cancelled", "CANCELLED"))
    }
    ...
    async fn create_customer(&self, _customer: Customer) -> PaymentResult<String> {
        Ok(format!("MPESA_CUS_{}", uuid::Uuid::new_v4()))
    }
    ...
    async fn attach_payment_method(
        ...
        Ok(format!("MPESA_PM_{}", uuid::Uuid::new_v4()))
    }
    ...
    async fn update_subscription(
        ...
        Ok(Subscription::new(
            ...
            "customer_123",
            &SubscriptionPlan::new("default", Amount::new(10000, "KES"), BillingInterval::Month),
        ))
    }
    ...
    async fn create_batch_payout(&self, payouts: Vec<Payout>) -> PaymentResult<BatchPayout> {
        Ok(BatchPayout::new(PaymentProvider::Mpesa, payouts))
    }
    ...
    async fn get_balance(&self, account_id: &str) -> PaymentResult<WalletBalance> {
        Ok(WalletBalance {
            ...
            available: Amount::new(500000, "KES"),
            pending: Amount::new(50000, "KES"),
