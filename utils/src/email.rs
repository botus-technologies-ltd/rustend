...
pub trait EmailSender: Send + Sync + 'static {
    ...
    async fn verify_email(&self, _email: &str) -> Result<bool, EmailError> {
        Ok(true)
    }
}
...
pub mod templates {
    ...
    pub fn welcome_email(to: &str, name: &str) -> Email {
        Email::new("noreply@example.com", to, "Welcome!").html(format!(
    ...
    pub fn order_confirmation(to: &str, order_id: &str, amount: &str) -> Email {
        Email::new("orders@example.com", to, format!("Order #{}", order_id)).html(format!(
            "<html><body><h1>Order Confirmed!</h1><p>Order: {}<br>Amount: {}</p></body></html>",
            order_id, amount
        ))
    }

    pub fn notification(to: &str, title: &str, message: &str) -> Email {
        Email::new("notifications@example.com", to, title).html(format!(
...
impl EmailSender for SmtpEmailSender {
    async fn send(&self, _email: &Email) -> EmailResult {
        EmailResult::success(format!("smtp_{}", uuid::Uuid::new_v4()))
    }
}
...
impl EmailSender for SendGridSender {
    async fn send(&self, _email: &Email) -> EmailResult {
        EmailResult::success(format!("sg_{}", uuid::Uuid::new_v4()))
    }
}
...
impl SesSender {
    pub fn new(region: impl Into<String>, from_email: impl Into<String>) -> Self {
        Self {
            _region: region.into(),
            _client: reqwest::Client::new(),
            _from_email: from_email.into(),
...
impl EmailSender for SesSender {
    async fn send(&self, _email: &Email) -> EmailResult {
        EmailResult::success(format!("ses_{}", uuid::Uuid::new_v4()))
    }
}
...
impl EmailSender for MailgunSender {
    async fn send(&self, _email: &Email) -> EmailResult {
        EmailResult::success(format!("mg_{}", uuid::Uuid::new_v4()))
    }
}

/// Email Service
pub struct EmailService {
    sender: Box<dyn EmailSender + Send + Sync + 'static>,
}
...
impl EmailService {
    pub fn from_config(config: EmailConfig) -> Result<Self, EmailError> {
        let sender: Box<dyn EmailSender + Send + Sync + 'static> = match config.provider {
            EmailProvider::Smtp => {
                let smtp = config
                    .smtp
                    .ok_or_else(|| EmailError::Config("SMTP config required".into()))?;
                Box::new(SmtpEmailSender::new(smtp))
            }
            EmailProvider::SendGrid => {
                let api = config
                    .api
                    .ok_or_else(|| EmailError::Config("API config required".into()))?;
            ...
            EmailProvider::Mailgun => {
                ...
                let domain = api
                    .endpoint
                    .ok_or_else(|| EmailError::Config("Domain required".into()))?;
                Box::new(MailgunSender::new(api.api_key, domain))
            }
            EmailProvider::HttpApi => {
                return Err(EmailError::Config("HTTP API not implemented".into()));
            }
        };
        Ok(Self { sender })
    }
    ...
    pub fn smtp(config: SmtpConfig) -> Self {
        Self {
            sender: Box::new(SmtpEmailSender::new(config)),
        }
    }
    pub fn sendgrid(api_key: impl Into<String>, from_email: impl Into<String>) -> Self {
        Self {
            sender: Box::new(SendGridSender::new(api_key, from_email)),
        }
    }
    pub fn ses(region: impl Into<String>, from_email: impl Into<String>) -> Self {
        Self {
            sender: Box::new(SesSender::new(region, from_email)),
        }
    }
    pub fn mailgun(api_key: impl Into<String>, domain: impl Into<String>) -> Self {
        Self {
            sender: Box::new(MailgunSender::new(api_key, domain)),
        }
    }

    pub async fn send(&self, email: &Email) -> EmailResult {
    ...
    pub async fn send_template<F>(&self, to: &str, template_fn: F) -> EmailResult
    where
        F: FnOnce(&str) -> Email,
    {
